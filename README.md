...
For this project we spent a good amount of time talking about the layout of the code and the specific use cases that we wanted to implement.  This then led to a big discusssion on the risks that were present in each of these cases.

One of our first big things we wanted to do for the first round was make the system completly offline.  This means that once the app is downloaded for the user, they can use it at any time or place.  This was big because we did not want to rely on servers for the first itteration.  If we were able to design a completly on device app we could then later integrate a server linking to this system to provide the capability of saving this data in the cloud.  The downside to the completly on device app is that if the users device were to break, the data from the app would most likely be lost.  We were willing to sacrafice this because the data that is here is fairly trivial and the way to further develop the app would be easier in the future.

One other risk that was encountered that the system we chose to handle storing the data was not easily testable through CI/CD automation.  We spent quite a few hours hammering out different attempts that people online suggested, but none of them were working as expected.  Becuase of this our code has a lack of confirmation on some functions that add and delete items from this system.  We were able to implement a good amount testing the expected reactions of users interactions on the screen but nothing directly interfacing with that database system.  In order to somewhat confidentently put this first revision into main we spent a lot of time testing different scenerios to see how the code reacted, which is known not to be reliable, but it was an attempt to make up for lack proficiecy in this tool.

Our main focus then was to create all the nescessities for someone to be able to plan their dinners out in advance.  This meant having a way to have recipes on device, link recipes to a given day, and then we really wanted a way to gather a grocery list to make shopping effiecient.  This narrowed the scope of our project for the first few sprints so we could get a fully functioning app out to customers in a timly fasion.

For this project, there is some things to do in the future. Now the software does not have a login/register page, because everything is stored on device. This needs to be created and connected to a server/database so users can access their **Dinner Planner App** data on more than just one device.  This would allow an entire family to be able to plan and see the meals that they will be eating.  The recipe page could be altered so that the user can edit a recipe instead of them being deleted and remade. Another thing that was not high on our priority list was the estetics of the app.  We wanted everything easily reachable and in a easy to understand manner, but the artistic styling did not get much effor.  Finally, adding the option to specifically plan meals for breakfast, lunch, or dinner on all days for added organization.
